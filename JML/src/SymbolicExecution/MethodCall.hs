...
import SymbolicExecution.Internal.Calculator (numericCalculator, booleanCalculator, stringCalculator)
      ...
      SymUnknown (_,vn,_) _ -> do
        ...
        case visited of
          ER_Expr ex -> do
            modify $ \symState ->
              SymState {
                env = Map.insert key ex (env symState),
                pc  = pc symState
              }
            let toReturn = ER_SymStateMapEntry key ex
            tell [Log.Return "visitSymExpr -> SymUnknown" (show toReturn)] $> toReturn
          ER_Void -> tell [Log.Skip $ "visitSymExpr0 -> SymUnknown -> VarName " ++ vn] $> ER_Void
  ------------------------------
  ------------------------------
  ------------------------------
{- TODELETE
...
{- TODELETE
 ...
 -}
  ...
  ------------------------------
      SVarBindings _ -> do
        tell [Log.SymExpr_2_Handle (show val) "visitSymExpr -> SVarBindings"]
        tell [Log.ModifyState "visitSymExpr -> SVarBindings" (show key,show val)]
        modify $ \symState ->
          SymState {
            env = Map.insert key val (env symState),
            pc  = pc symState
        ...
        let toReturn = ER_SymStateMapEntry key val
        tell [Log.Return "visitSymExpr -> SVarBindings" (show toReturn)] $> toReturn
  ------------------------------
  ------------------------------
  ------------------------------
      SymString _ -> do
...
{- TODELETE
 ...
 -}
    ...
    toReturn <- (\e1 e2 ->
      let isNumericOp = symBinOp `elem` [Add, Mul, Sub, Div, Mod]
          whichFun = case isNumericOp of
            True | all isSymString [symExpr1,symExpr2] -> stringCalculator
                 | otherwise -> numericCalculator
            False -> booleanCalculator
      in case (e1,e2) of
           (ER_Expr _,ER_Expr _) ->
              let Just e1_ = getSymExpr e1
                  Just e2_ = getSymExpr e2
  ...
  val@(SymString _) -> do
    tell [Log.SymExpr_2_Handle (show val) "visitSymExpr0 -> SymString"]
    let toReturn = ER_Expr val
    tell [Log.Return "visitSymExpr0 -> SymString" (show toReturn)] $> toReturn
    ...
    case mExpr of
      ...
      Just symExpr2 -> do
        ...
        let newVal = case visited of
              ER_Expr ex -> ex
            toReturn = ER_Expr newVal
        tell [Log.Return "visitSymExpr0 -> SymUnknown Just" (show toReturn)] $> toReturn
  ------------------------------
  ------------------------------
  ------------------------------
  val@(SymNum _) -> do
    tell [Log.SymExpr_2_Handle (show val) "visitSymExpr0 -> SymNum"]
    let toReturn = ER_Expr val
    tell [Log.Return "visitSymExpr0 -> SymNum" (show toReturn)] $> toReturn
  ------------------------------
  ------------------------------
  ------------------------------
  ex ->
