...
===Begin Logs===
================
1. Next Node: Entry (BuiltInType Int) "boo28_p" []

>>>>>>>>>> visitNode <<<<<<<<<<

2. (visitNode -> Entry): Method Start: boo28_p

3. (visitNode -> Entry -> method with no args): Returning: ()

4. (visitNode -> Entry -> method has args): Returning: ER_State (SymState {env = fromList [(MethodName "boo28_p",SMethodType Int)], pc = []})
...
153. actual: boo28 ==> (visitSymExpr -> SBin): Returning: ER_Expr (SBool True)

154. actual: boo28 ==> Next symExpr (MethodName "boo28" ==> SMethodType Int) in Method Call: boo28

155. actual: boo28 ==> >>>>>>>>>> visitSymExpr <<<<<<<<<<

156. actual: boo28 ==> (visitSymExpr -> SMethodType): handling SymExpr: SMethodType Int

157. actual: boo28 ==> (visitSymExpr -> SMethodType): Modifying State: (MethodName "boo28",SMethodType Int)

158. actual: boo28 ==> (visitSymExpr -> SMethodType): Returning: ER_SymStateMapEntry {er_key = MethodName "boo28", er_val = SMethodType Int}

159. actual: boo28 ==> Next symExpr (FormalParms ==> SFormalParms ["i"]) in Method Call: boo28

160. actual: boo28 ==> >>>>>>>>>> visitSymExpr <<<<<<<<<<

161. actual: boo28 ==> (visitSymExpr -> SFormalParms): handling SymExpr: SFormalParms ["i"]

162. actual: boo28 ==> (visitSymExpr -> SFormalParms): Modifying State: (FormalParms,SFormalParms ["i"])

163. actual: boo28 ==> (visitSymExpr -> SFormalParms): Returning: ER_SymStateMapEntry {er_key = FormalParms, er_val = SFormalParms ["i"]}

164. actual: boo28 ==> Next symExpr (VarBindings ==> SVarBindings (fromList [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}})])) in Method Call: boo28

165. actual: boo28 ==> >>>>>>>>>> visitSymExpr <<<<<<<<<<

166. actual: boo28 ==> (visitSymExpr -> SVarBindings): handling SymExpr: SVarBindings (fromList [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}})])

167. actual: boo28 ==> (visitSymExpr -> SVarBindings): Modifying State: (VarBindings,SVarBindings (fromList [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}})]))

168. actual: boo28 ==> (visitSymExpr -> SVarBindings): Returning: ER_SymStateMapEntry {er_key = VarBindings, er_val = SVarBindings (fromList [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}})])}

169. actual: boo28 ==> Next symExpr (VarAssignments ==> SVarAssignments [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}}),("x",Node_Coor {varDeclAt = 3, varFrame = SR {branchStart = 2, branchEnd = 5}})]) in Method Call: boo28

170. actual: boo28 ==> >>>>>>>>>> visitSymExpr <<<<<<<<<<

171. actual: boo28 ==> (visitSymExpr -> SVarAssignments): handling SymExpr: SVarAssignments [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}}),("x",Node_Coor {varDeclAt = 3, varFrame = SR {branchStart = 2, branchEnd = 5}})]

172. actual: boo28 ==> (visitSymExpr -> SVarAssignments): Modifying State: (VarAssignments,SVarAssignments [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}}),("x",Node_Coor {varDeclAt = 3, varFrame = SR {branchStart = 2, branchEnd = 5}})])

173. actual: boo28 ==> (visitSymExpr -> SVarAssignments): Returning: ER_SymStateMapEntry {er_key = VarAssignments, er_val = SVarAssignments [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}}),("x",Node_Coor {varDeclAt = 3, varFrame = SR {branchStart = 2, branchEnd = 5}})]}

174. actual: boo28 ==> Next symExpr (VarName "i" ==> SymVar Int "i") in Method Call: boo28

175. actual: boo28 ==> >>>>>>>>>> visitSymExpr <<<<<<<<<<

176. actual: boo28 ==> (visitSymExpr -> SymVar -> Formal Parameter): handling SymExpr: SymVar Int "i"

177. actual: boo28 ==> (visitSymExpr0 -> SymVar -> Formal Parameter): handling SymExpr: SymVar Int "i"

178. actual: boo28 ==> (visitSymExpr0 -> SymVar): Returning: ER_Expr (SymInt 10)

179. actual: boo28 ==> (visitSymExpr -> SymVar): Modifying State: (VarName "i",SymInt 10)

180. actual: boo28 ==> (visitSymExpr -> SymVar): Returning: ER_SymStateMapEntry {er_key = VarName "i", er_val = SymInt 10}

181. actual: boo28 ==> Next symExpr (VarName "x" ==> SymInt 2) in Method Call: boo28

182. actual: boo28 ==> >>>>>>>>>> visitSymExpr <<<<<<<<<<

183. actual: boo28 ==> (visitSymExpr -> SymInt): handling SymExpr: SymInt 2

184. actual: boo28 ==> (visitSymExpr -> SymInt): Modifying State: (VarName "x",SymInt 2)

185. actual: boo28 ==> (visitSymExpr -> SymInt): Returning: ER_SymStateMapEntry {er_key = VarName "x", er_val = SymInt 2}

186. actual: boo28 ==> Next symExpr (Return ==> SymVar Int "i") in Method Call: boo28

187. actual: boo28 ==> >>>>>>>>>> visitSymExpr <<<<<<<<<<

188. actual: boo28 ==> (visitSymExpr -> SymVar -> Formal Parameter): handling SymExpr: SymVar Int "i"

189. actual: boo28 ==> (visitSymExpr0 -> SymVar -> Formal Parameter): handling SymExpr: SymVar Int "i"

190. actual: boo28 ==> (visitSymExpr0 -> SymVar): Returning: ER_Expr (SymInt 10)

191. actual: boo28 ==> (visitSymExpr -> SymVar): Modifying State: (Return,SymInt 10)

192. actual: boo28 ==> (visitSymExpr -> SymVar): Returning: ER_SymStateMapEntry {er_key = Return, er_val = SymInt 10}

193. actual: boo28 ==> (visitSymExpr -> SIte -> resolved condition is True -> else body exists): Modifying State: (<no key>,<whole state is updated>: SymState {env = fromList [(MethodName "boo28",SMethodType Int),(FormalParms,SFormalParms ["i"]),(VarBindings,SVarBindings (fromList [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}})])),(VarAssignments,SVarAssignments [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}}),("x",Node_Coor {varDeclAt = 3, varFrame = SR {branchStart = 2, branchEnd = 5}})]),(VarName "i",SymInt 10),(VarName "x",SymInt 2),(Return,SymInt 10)], pc = []})

194. actual: boo28 ==> Next symExpr (Return ==> SymInt 5) in Method Call: boo28

195. actual: boo28 ==> (Skip):
"(Return,SymInt 5)"

196. Method Call actual SymState: SymState {env = fromList [(MethodName "boo28",SMethodType Int),(FormalParms,SFormalParms ["i"]),(VarBindings,SVarBindings (fromList [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}})])),(VarAssignments,SVarAssignments [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}}),("x",Node_Coor {varDeclAt = 3, varFrame = SR {branchStart = 2, branchEnd = 5}})]),(VarName "i",SymInt 10),(VarName "x",SymInt 2),(Return,SymInt 10)], pc = []}

197. (visitExpr -> FunCallExpr -> with parameters): Returning: ER_FunCall (SymState {env = fromList [(MethodName "boo28",SMethodType Int),(FormalParms,SFormalParms ["i"]),(VarBindings,SVarBindings (fromList [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}})])),(VarAssignments,SVarAssignments [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}}),("x",Node_Coor {varDeclAt = 3, varFrame = SR {branchStart = 2, branchEnd = 5}})]),(VarName "i",SymInt 10),(VarName "x",SymInt 2),(Return,SymInt 10)], pc = []})

198. (visitStmt -> ReturnStmt -> method with args): Modifying State: (return,SymInt 10)

199. (visitStmt -> ReturnStmt): Returning: ER_FunCall (SymState {env = fromList [(MethodName "boo28",SMethodType Int),(FormalParms,SFormalParms ["i"]),(VarBindings,SVarBindings (fromList [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}})])),(VarAssignments,SVarAssignments [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}}),("x",Node_Coor {varDeclAt = 3, varFrame = SR {branchStart = 2, branchEnd = 5}})]),(VarName "i",SymInt 10),(VarName "x",SymInt 2),(Return,SymInt 10)], pc = []})

200. (visitNode -> End -> method returns): Returning: ER_FunCall (SymState {env = fromList [(MethodName "boo28",SMethodType Int),(FormalParms,SFormalParms ["i"]),(VarBindings,SVarBindings (fromList [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}})])),(VarAssignments,SVarAssignments [("x",Node_Coor {varDeclAt = 1, varFrame = SR {branchStart = 0, branchEnd = 4}}),("x",Node_Coor {varDeclAt = 3, varFrame = SR {branchStart = 2, branchEnd = 5}})]),(VarName "i",SymInt 10),(VarName "x",SymInt 2),(Return,SymInt 10)], pc = []})
==============
===End Logs===
==============
...
SymState:
SymState {env = fromList [(MethodName "boo28_p",SMethodType Int),(Return,SymInt 10)], pc = []}