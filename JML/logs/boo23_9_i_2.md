...
===Begin Logs===
================
1. Next Node: Entry (BuiltInType Int) "boo23_9_i_2" [VarExpr {varType = Just (BuiltInType Int), varObj = [], varName = "i"}]

>>>>>>>>>> visitNode <<<<<<<<<<

2. (visitNode -> Entry): Method Start: boo23_9_i_2
...
13. (visitNode -> Entry -> method has args): Returning: ER_State (SymState {env = fromList [(MethodName "boo23_9_i_2",SMethodType Int),(FormalParms,SFormalParms ["i"]),(VarName "i",SymVar Int "i")], pc = []})
...
97. (visitExpr -> BinOpExpr): handling expression: BinOpExpr {expr1 = NumberLiteral 2.0, binOp = *, expr2 = VarExpr {varType = Nothing, varObj = [], varName = "i"}}

98. (visitExpr -> NumberLiteral): handling expression: NumberLiteral 2.0

99. (visitExpr -> NumberLiteral): Returning: ER_Expr (SymNum 2.0)

100. (visitExpr -> VarExpr): handling expression: VarExpr {varType = Nothing, varObj = [], varName = "i"}

101. (visitExpr -> VarExpr): Look up in environmane table: (i ~~> SymVar Int "i") 

102. (visitExpr -> VarExpr -> Updating): Returning: ER_SymStateMapEntry {er_key = VarName "i", er_val = SymVar Int "i"}

103. (visitExpr -> BinOpExpr): Affected: SymNum 2.0, *, SymVar Int "i"

104. (visitExpr -> BinOpExpr -> numericCalculator): Returning: ER_Expr (SBin (SymInt 2) Mul (SymVar Int "i"))

105. (visitExpr -> BinOpExpr): Affected: SBin (SymInt 5) Add (SymVar Int "i"), -, SBin (SymInt 2) Mul (SymVar Int "i")

106. (visitExpr -> BinOpExpr -> numericCalculator): Returning: ER_Expr (SBin (SymInt 5) Sub (SymVar Int "i"))

107. (visitExpr -> AssignExpr): Affected: ER_SymStateMapEntry {er_key = VarName "x", er_val = SymNull Int}, ER_Expr (SBin (SymInt 5) Sub (SymVar Int "i"))

108. (visitExpr ==> AssignExpr): Update Variable
    Var Name: VarName "x"
    Old Value: SymNull Int
    New Value: SBin (SymInt 5) Sub (SymVar Int "i")

109. (visitExpr ==> AssignExpr): Modifying State: (VarName "x",SBin (SymInt 5) Sub (SymVar Int "i"))

110. (visitExpr -> AssignExpr): Returning: ER_SymStateMapEntry {er_key = VarName "x", er_val = SBin (SymInt 5) Sub (SymVar Int "i")}

111. (visitStmt -> AssignStmt): Returning: ER_SymStateMapEntry {er_key = VarName "x", er_val = SBin (SymInt 5) Sub (SymVar Int "i")}

112. (visitNode -> Node -> Statement): Adding Var Assignment: ("x",Node_Coor {varDeclAt = 2, varFrame = SR {branchStart = 0, branchEnd = 3}})

113. (visitNode -> Node -> Statement): Returning: ER_SymStateMapEntry {er_key = VarName "x", er_val = SBin (SymInt 5) Sub (SymVar Int "i")}

114. Next Node: End {id = 3, parent = 0, mExpr = Just (VarExpr {varType = Nothing, varObj = [], varName = "x"})}

>>>>>>>>>> visitNode <<<<<<<<<<

115. (visitNode -> End): Method End

116. (visitNode -> End -> return something): handling return expression: VarExpr {varType = Nothing, varObj = [], varName = "x"}

117. (visitStmt -> ReturnStmt): handling return expression: VarExpr {varType = Nothing, varObj = [], varName = "x"}

118. (visitExpr -> VarExpr): handling expression: VarExpr {varType = Nothing, varObj = [], varName = "x"}

119. (visitExpr -> VarExpr): Look up in environmane table: (x ~~> SBin (SymInt 5) Sub (SymVar Int "i")) 

120. (visitExpr -> VarExpr -> Updating): Returning: ER_SymStateMapEntry {er_key = VarName "x", er_val = SBin (SymInt 5) Sub (SymVar Int "i")}

121. (visitStmt -> ReturnStmt -> method with args): Modifying State: (return,SBin (SymInt 5) Sub (SymVar Int "i"))

122. (visitStmt -> ReturnStmt): Returning: ER_SymStateMapEntry {er_key = VarName "x", er_val = SBin (SymInt 5) Sub (SymVar Int "i")}

123. (visitNode -> End -> method returns): Returning: ER_SymStateMapEntry {er_key = VarName "x", er_val = SBin (SymInt 5) Sub (SymVar Int "i")}
==============
===End Logs===
==============
